    ...
    <div class="stack" :class="getClass" :style="getStyle">
        <div :class="getSplitClass" class="fill-full-width">
            <template v-if="buttonLeftSide">
                <Button class="mr-2" :color="!show ? 'green' : 'light-green'" @click="show = !show">
                    <Icon :size="14" :icon="show ? 'icon-minus' : 'icon-plus'"></Icon>
                </Button>
            </template>
            <span class="overline module-text" :class="show ? 'green--text' : 'grey--text'">
                {{ name }}
            </span>
            <template v-if="!buttonLeftSide">
                <Button class="ml-2" :color="!show ? 'green' : 'light-green'" @click="show = !show">
                    <Icon :size="14" :icon="show ? 'icon-minus' : 'icon-plus'"></Icon>
                </Button>
            </template>
        </div>
