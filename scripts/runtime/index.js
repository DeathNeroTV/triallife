...
if (NO_SPECIAL_CHARACTERS.test(process.cwd())) {
    console.warn(`Hey! A folder in your triallife path has special characters in it.`);
...
async function refreshFileWatching() {
    ...
    // ignore `/triallife/server` && `/triallife/client` directories
    previousGlobFiles = files.filter((fileName) => {
        if (fileName.includes('/triallife/server/')) {
            return false;
        }

        if (fileName.includes('/triallife/client/')) {
            return false;
        }
