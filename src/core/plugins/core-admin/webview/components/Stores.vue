<template>
    <div class="wrapper split-center no-overflow">
        <div class="split split-center fill-full-width pt-2 pb-2 outlined">
            <span class="split split-center fill-full-width center pt-2 pb-2 text-lg-h5">{{ title }}</span>
            <Button class="mr-2" color="green" :glow="true" :raise="true" @click="e => $emit('modify-data', 'stores', this.store)">
        ...
        <div class="list">
            <table class="fill-full-width">
                <thead class="grey darken-4" style="text-align: center">
                    <tr>
                        <th class="pl-5 pr-5 pt-2 pb-2" v-for="(key, index) in Object.keys(store)">
                            {{ key.toUpperCase() }}
                        </th>
                ...
                <tbody>
                    <tr v-for="(shop, index) in list" :class="'grey ' + (index % 2 === 0 ? 'darken-2' : 'darken-3')" style="text-align: center">
                        <td v-for="(key, idx) in Object.keys(shop)">
                            {{ typeof shop[key] === 'object' || Array.isArray(shop[key]) ? JSON.stringify(shop[key]) : shop[key].toString() }}
                        </td>
