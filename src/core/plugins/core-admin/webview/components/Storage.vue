<template>
     <div class="wrapper split-center no-overflow">
        <div class="split split-center fill-full-width pt-2 pb-2 mb-2 outlined">
            ...
            <Button class="mr-2" color="green" :glow="true" :raise="true" @click="e => $emit('modify-data', 'storage', this.storage)">
        ...
        <div class="list">
            <table class="fill-full-width">
                <thead class="grey darken-4" style="text-align: center">
                    <tr>
                        <th class="pl-5 pr-5 pt-2 pb-2" v-for="(key, index) in Object.keys(storage)">
                            {{ key.toUpperCase() }}
                        </th>
                ...
                <tbody>
                    <tr v-for="(warehouse, index) in list" :class="'grey ' + (index % 2 === 0 ? 'darken-2' : 'darken-3')" style="text-align: center">
                        <td v-for="(key, idx) in Object.keys(warehouse)">
                            {{ typeof warehouse[key] === 'object' || Array.isArray(warehouse[key]) ? JSON.stringify(warehouse[key]) : warehouse[key].toString() }}
                        </td>
