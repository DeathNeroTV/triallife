...
export class HudView {
    ...
    static init() {
        ...
        KeybindController.registerKeybind({ key: alt.Voice.activationKey, singlePress: InternalFunctions.voiceUp });
    }
    ...
    private static toggleKeyBind() {
        ...
        triallifeClient.webview.emit(`${PAGE_NAME}:SwitchHudState`, hudStates[hudStateIndex]);
    }
     ...
     *
     * @static
     * @param {IClientInteraction} _interaction
     * @memberof HudView
     */
     ...
     *
     * @static
     * @param {Array<IClientInteraction>} _interactions
     * @memberof HudView
     */
     ...
     * @static
     * @param {string} uid
     * @param {(propName: string) => void} callback
...
class InternalFunctions implements ViewModel {
    ...
    static metaChange(key: string, value: any) {
        if (key === 'voice' && alt.Voice.voiceControlsEnabled && alt.Player.local.meta.voice) {
            InternalFunctions.passComponentInfo(HUD_COMPONENT.MICROPHONE, 0);
        }
    }
    ...
    static async passComponentInfo(propName: string, value: any, isJSON = false) {
        triallifeClient.webview.emit(`${PAGE_NAME}:SetProp`, propName, value, isJSON);
    }
    ...
    static defaultWaterComponent(propName: string) {
        ...
        InternalFunctions.passComponentInfo(propName, water !== undefined && water !== null ? parseInt(water.toFixed(0)) : 100);
    }

    static defaultFoodComponent(propName: string) {
        const food = alt.Player.local.meta.food;
        InternalFunctions.passComponentInfo(propName, food !== undefined && food !== null ? parseInt(food.toFixed(0)) : 100);
    }
    ...
    static defaultMicrophoneComponent(propName: string) {
        const microphoneVolume = alt.Player.local.meta.voiceRange;
        InternalFunctions.passComponentInfo(propName, microphoneVolume && microphoneVolume > 0 ? (microphoneVolume / 32) * 100 : 0);
    }

    static voiceDown() {} //placeholder for internal handling

    static voiceUp() {
        native.playSoundFrontend(-1, 'Input_Code_Up', 'Safe_Minigame_Sounds', true);
        alt.emitServer('voice:ChangeRange');
    }

    static defaultIdentifier(propName: string) {
        InternalFunctions.passComponentInfo(propName, alt.Player.local.getSyncedMeta(PLAYER_SYNCED_META.IDENTIFICATION_ID));
    }
}
