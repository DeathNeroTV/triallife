...
export class ExampleWebViewServer {
    ...
    private static handleRefresh(player: alt.Player) {
        ...
        triallife.webview.emit(player, ExampleWebViewEvents.ServerToWebView.REFRESH_PLAYERS, ExampleWebViewServer.getAvailablePlayers());
    }
    ...
    private static handleKickPlayer(player: alt.Player, idToKick: number) {
        ...
        const target = triallife.systems.identifier.getPlayer(idToKick);
    ...
    private static getAvailablePlayers(): Array<ExPlayerInfo> {
        return [...alt.Player.all]
            ...
            .map((t) => {
                const id = triallife.systems.identifier.getIdByStrategy(t);
                return {
                    id,
                    name: t.data.name,
                    ping: t.ping,
