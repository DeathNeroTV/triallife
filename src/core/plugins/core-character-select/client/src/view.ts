...
class InternalFunctions implements ViewModel {
    static async open(_characters: Partial<Character>[]) {
        ...
        await PedCharacter.create(_characters[0].appearance.sex === 1 ? true : false, CHARACTER_SELECT_CONFIG.CHARACTER_SELECT_POS, CHARACTER_SELECT_CONFIG.CHARACTER_SELECT_ROT);
    ...
    static async update(index: number) {
        ...
        await new Promise((resolve: Function) => {
            ...
            const interval = alt.setInterval(() => {
                ...
                playPedAnimation(PedCharacter.get(), IDLE_ANIM_DICT, IDLE_ANIM, ANIMATION_FLAGS.NORMAL | ANIMATION_FLAGS.REPEAT);
