...
class InternalFunctions {
    ...
    static select(player: Player, id: string): void {
        ...
        if (index <= -1) {
            alt.log(`~lb~3LRP ==> ~w~Charakter nicht gefunden. Spieler ${player.id} zur Charakterauswahl zurück gesendet.`);
        ...
        if (CharacterList[player.id][index].banned.state) {
            ...
            alt.log(`~lb~3LRP ==> ~w~Charakter ist gebannt. Spieler ${player.id} zur Charakterauswahl zurück gesendet.`);
            return;
        }
