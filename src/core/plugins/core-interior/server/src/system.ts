...
import { databaseConst } from '../../../../server/api/consts/constDatabase';
...
class InternalSystem {
     ...
     * @memberof InteriorSystem
     */
    static async setName(player: alt.Player, uid: string, name: string, skipDistanceCheck = false, skipOwnerCheck = false) {
        ...
        if (!skipDistanceCheck) {
            ...
            if (dist >= 5) {
                triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_TOO_FAR);
                return;
            }
        }
        ...
        if (skipOwnerCheck) {
            if (!InteriorSystem.isOwner(player, interior)) {
                triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_NO_KEYS);
                triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
                return;
            }
        }
     ...
     * @memberof InteriorSystem
     */
    static async setPrice(player: alt.Player, uid: string, value: number, skipDistanceCheck = false, skipOwnerCheck = false) {
        if (!player || !player.valid || player.data.isDead || !uid) {
            return;
        }
    ...
    static async storage(player: alt.Player, uid: string) {
        ...
        if (!isFlagEnabled(interior.system, INTERIOR_SYSTEM.HAS_STORAGE)) {
            triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_NO_STORAGE);
            return;
        }
    ...
    static async purchase(player: alt.Player, uid: string, skipDistanceCheck = false, skipOwnerCheck = false) {
        ...
        if (!triallife.player.currency.subAllCurrencies(player, interior.price)) {
            triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_NOT_ENOUGH_CURRENCY);
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            return;
        }
        ...
        triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_DID_PURCHASE_INTERIOR);
...
export class InteriorSystem {
    ...
    static async movePlayerIn(player: alt.Player, uid: string, noTeleport = false, skipDistanceCheck = false): Promise<boolean> {
        const interior = await InteriorSystem.get(uid);
        ...
        if (isFlagEnabled(interior.system, INTERIOR_SYSTEM.HAS_LOCK) && !interior.isUnlocked) {
            triallife.player.emit.notification(player, LOCALE_INTERIOR_VIEW.LABEL_DOOR_IS_LOCKED);
            return false;
        }
        ...
        triallife.player.safe.setDimension(player, interior.dimension);
        ...
        if (!noTeleport) {
            ...
            triallife.player.safe.setPosition(player, interior.inside.x, interior.inside.y, interior.inside.z + 1);
        ...
        triallife.state.set(player, 'interior', interior.uid);
    ...
    static async movePlayerOut(player: alt.Player, skipDistanceCheck = false): Promise<boolean> {
        ...
        if (!interior) {
            ...
            triallife.player.safe.setPosition(player, LOST_PLAYER_POS.x, LOST_PLAYER_POS.y, LOST_PLAYER_POS.z);
            return false;
        }
        ...
        triallife.player.set.frozen(player, true);
        triallife.player.safe.setDimension(player, 0);
        triallife.player.safe.setPosition(player, interior.outside.x, interior.outside.y, interior.outside.z + 1);
        triallife.state.set(player, 'interior', null);
        alt.setTimeout(() => {
            if (!player || !player.valid) {
                return;
            }
    ...
    static hasAccess(player: alt.Player, interior: Interior): boolean {
        ...
        if (interior.keys) {
            for (let i = 0; i < interior.keys.length; i++) {
                const isValidKey = triallife.player.inventory.checkForKeyValuePair(player, 'key', interior.keys);
                if (!isValidKey) {
                    continue;
                }
    ...
    static async addFactionOwnership(uid: string, faction: string): Promise<boolean> {
        ...
        await Database.updatePartialData(interior._id.toString(), { factions: interior.factions }, Collections.Interiors);
...
PlayerEvents.on(TRIALLIFE_EVENTS_PLAYER.SELECTED_CHARACTER, InternalSystem.spawn);
