...
class InternalFunctions implements ViewModel {
    static async open(_storeData: IClothingStore, _appearance: Appearance, _equipment: Array<Item>) {
        ...
        await PedCharacter.create(appearance.sex === 1 ? true : false, alt.Player.local.pos, native.getEntityHeading(alt.Player.local.scriptID));
     ...
     * @memberof InternalFunctions
     */
    static purchase(uid: string, index: number, component: ClothingComponent, name: string, desc: string, noSound = false) {
        ...
        for (let i = 0; i < components.length; i++) {
            ...
            for (let index = 0; index < component.drawables.length; index++) {
                ...
                if (component.isProp) {
                    ...
                    maxDrawables = CLOTHING_CONFIG.MAXIMUM_PROP_VALUES[appearance.sex][id] + InternalFunctions.getDlcClothingCount(appearance.sex, id, true);
                    ...
                    maxDrawables = CLOTHING_CONFIG.MAXIMUM_COMPONENT_VALUES[appearance.sex][id] + InternalFunctions.getDlcClothingCount(appearance.sex, id, false);
