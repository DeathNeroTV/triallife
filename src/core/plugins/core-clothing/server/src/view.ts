...
// Do not change order
const icons = ['hat', 'mask', 'shirt', 'bottoms', 'shoes', 'glasses', 'ear', 'backpack', 'armour', 'watch', 'bracelet', 'accessory'];
...
export class ClothingFunctions {
    ...
    static init() {
        for (let i = 0; i < clothingStores.length; i++) {
            ...
            ClothingFunctions.create(position, clothingData, deepCloneObject(defaultBlip), deepCloneObject(defaultInteraction));
        }
     ...
     * @memberof ClothingFunctions
     */
    static getDlcHash(player: alt.Player, id: number, drawable: number, texture: number, isProp: boolean = false): { dlcName: string | number; drawable: number; texture: number } {
        ...
        const dataSet: Array<CLOTHING_DLC_INFO> = isProp ? CLOTHING_CONFIG.DLC_PROPS[id] : CLOTHING_CONFIG.DLC_CLOTHING[id];
    ...
    static async purchaseAll(player: alt.Player, components: Array<ComponentVueInfo>) {
        ...
        for (const comp of components) {
            const result = await ClothingFunctions.purchase(player, comp.uid, comp.index, comp.componentData, comp.pageName, comp.name, true);
     ...
     * @memberof ClothingFunctions
     */
    static async purchase(player: alt.Player, shopUID: string, equipmentSlot: number, component: ClothingComponent, name: string, desc: string, noSound = false): Promise<boolean> {
        ...
        triallife.player.sync.singleEquipment(player, newItem.data as ClothingComponent);

        await alt.Utils.wait(500);

        for (let i = 0; i < component.drawables.length; i++) {
            const dlcInfo = ClothingFunctions.getDlcHash(player, component.ids[i], component.drawables[i], component.textures[i], component.isProp);
        ...
        if (triallife.player.inventory.isEquipmentSlotFree(player, equipmentSlot)) {
            didGetAdded = triallife.player.inventory.equipmentAdd(player, newItem, equipmentSlot);
        } else {
            ...
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.CLOTHING_ITEM_IN_INVENTORY));
        ...
        await triallife.state.setBulk(player, { inventory: player.data.inventory, equipment: player.data.equipment }, true);
        triallife.player.sync.inventory(player);
        triallife.player.sync.equipment(player, player.data.equipment as Item[], player.data.appearance.sex === 1);
