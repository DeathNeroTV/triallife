...
export class FactionFuncs {
    ...
    static async addRank(faction: Faction, newName: string, weight: number): Promise<boolean> {
        ...
        faction.ranks.push({
            ...
            vehicles: [],
            weight,
            uid: triallife.utility.hash.sha256Random(JSON.stringify(faction.ranks)),
        });
     ...
     * @memberof FactionFuncs
     */
    static async updateRankPermissions(faction: Faction, rankUid: string, rankPermissions: RankPermissions): Promise<boolean> {
        const index = faction.ranks.findIndex((r) => r.uid === rankUid);
        if (index <= -1) {
            return false;
        }
