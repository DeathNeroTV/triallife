...
import { triallifeScreenshot } from '../../../../server/utility/screenshot';
...
export class ConsoleCommands {
    ...
    static async setAdmin(discord: string, permissionLevel: string | number) {
        ...
        if (!player) {
            const result = await Database.updateDataByFieldMatch('discord', discord, { permissionLevel: permission }, Collections.Accounts);
            if (!result) alt.logWarning(`That player is not currently logged in or has no account on the server`);
            else alt.log(`(${discord}) had their permission level changed to: ${permissionLevel}.`);
            return;
        }

        player.accountData.permissionLevel = permission;
        await Database.updatePartialData(player.accountData._id, { ['permissionLevel']: player.accountData.permissionLevel }, Collections.Accounts);
    ...
    static kick(discordNameIDCatchAll: string, ...args: any[]) {
        ...
        if (discordNameIDCatchAll.length <= 14 && !isNaN(parseInt(discordNameIDCatchAll))) {
            const playerById = triallife.systems.identifier.getPlayer(discordNameIDCatchAll);
    ...
    static async ban(discord_or_id: string, ...args: any[]) {
        ...
        if (discord_or_id.length <= 14) {
            player = triallife.systems.identifier.getPlayer(discord_or_id);
        } else {
    ...
    static async screenshot(id: string) {
        ...
        let player: alt.Player;
        const playerById = triallife.systems.identifier.getPlayer(id);
        if (playerById && playerById.valid) {
            player = playerById;
        }
        ...
        const screenshot = await triallifeScreenshot.takeScreenshot(player);
        if (!screenshot) {
    ...
    static async dox(id: string) {
        ...
        if (!player) {
            const result = await Database.fetchData('discord', id, Collections.Accounts);
            if (!result) {
                alt.logWarning(`That player is not currently logged in or has no account on the server`);
                return;
            }
            alt.log(`=== Player Account (${id}) ===`);
            Object.keys(result).forEach((key) => alt.log(`${key}: ${JSON.stringify(result[key])}`));
            return;
        }
