...
class TestCommands {
    ...
    private static testErrorScreen(player: alt.Player) {
        triallife.player.emit.createErrorScreen(player, { duration: 5000, title: 'Test', text: 'Hello World!' });
    }
    ...
    private static testCreditsCommand(player: alt.Player) {
        triallife.player.emit.createCredits(player, {
            duration: 5000,
            role: 'triallife Creator',
            name: 'Stuyk',
        });
    }
    ...
    private static testObjectAttachCommand(player: alt.Player) {
        ...
        triallife.player.emit.objectAttach(player, attachable, 5000);
    }
    ...
    private static testObjectAttachInfiniteCommand(player: alt.Player) {
        triallife.player.emit.notification(player, `Will last for about 5 minutes~`);
        ...
        triallife.player.emit.objectAttach(player, attachable, 60000 * 5);
    }
    ...
    private static testJobMenuCommand(player: alt.Player) {
        const trigger: JobTrigger = {
            ...
            acceptCallback: (player: alt.Player) => {
                triallife.player.emit.notification(player, `You accepted!`);
            },
            cancelCallback: (player: alt.Player) => {
                triallife.player.emit.notification(player, `You declined!`);
            },
        };
    ...
    private static testActionMenuCommand(player: alt.Player) {
        ...
        triallife.player.set.actionMenu(
            player,
            // The Menu
            {
...
alt.onClient('hello:From:Client', (player) => {
    triallife.player.emit.notification(player, `Got menu option from client.`);
});

alt.onClient('animation:Action:Server', (player, data: Animation) => {
    if (!data) return;

    triallife.player.emit.animation(player, data.dict, data.name, data.flags, data.duration);
});
