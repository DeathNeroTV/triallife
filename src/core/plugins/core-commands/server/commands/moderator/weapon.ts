...
class WeaponCommands {
    ...
    private static handleCommand(player: alt.Player, weaponName: string): void {
        const inv = triallife.player.inventory.getFreeInventorySlot(player);

        if (inv === null || inv.slot === null) {
            triallife.player.emit.notification(player, `No room in first inventory tab.`);
            return;
        }

        const weapon = getWeaponByName(weaponName);
        if (!weapon) {
            triallife.player.emit.notification(player, `Weapon does not exist.`);
            return;
        }
        ...
        triallife.player.inventory.inventoryAdd(player, newItem, inv.slot);
        triallife.state.set(player, 'inventory', player.data.inventory, true);
        triallife.player.sync.inventory(player);
        triallife.player.emit.notification(player, `Added weapon: ${weapon.name}`);
    }
    ...
    @command('removeallweapons', LocaleController.get(LOCALE_KEYS.COMMAND_REMOVE_ALL_WEAPONS, '/removeallweapons'), PERMISSIONS.ADMIN)
    private static handleRemoveWeapons(player: alt.Player): void {
        const weps = triallife.player.inventory.removeAllWeapons(player);
        triallife.player.emit.notification(player, `Removed: ${weps.length} weapons`);
    }
}
