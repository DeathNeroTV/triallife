...
class NoClipCommand {
    ...
    private static handleCommand(player: alt.Player): void {
        ...
        if (!isNoClipping && !player.data.isDead) {
            ...
            triallife.player.emit.notification(player, `No Clip: ${LocaleController.get(LOCALE_KEYS.LABEL_ON)}`);
        ...
        triallife.player.emit.notification(player, `No Clip: ${LocaleController.get(LOCALE_KEYS.LABEL_OFF)}`);
        player.visible = true;
        player.health = 199;
    }
