...
class VehicleCommands {
    ...
    private static refillVehicleCommand(player: alt.Player) {
        ...
        triallife.vehicle.funcs.save(player.vehicle, player.vehicle.data);
        triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_REFILLED));
    }
    ...
    private static repairVehicleCommand(player: alt.Player) {
        const vehicle = player.vehicle ? player.vehicle : getClosestEntity<alt.Vehicle>(player.pos, player.rot, alt.Vehicle.all, 2);
        if (!vehicle) {
            return;
        }
        triallife.vehicle.funcs.repair(vehicle);
        if (!vehicle.data) {
            return;
        }
        vehicle.data.bodyHealth = 1000;
        vehicle.data.engineHealth = 1000;
        triallife.vehicle.funcs.save(vehicle, vehicle.data);
        triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_REPAIRED));
    }
    ...
    private static createTemporaryVehicleCommand(player: alt.Player, model: string): void {
        ...
        if (player.data.isDead) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.CANNOT_PERFORM_WHILE_DEAD));
            return;
        }

        const fwd = triallife.utility.vector.getVectorInFrontOfPlayer(player, 5);

        try {
            triallife.vehicle.funcs.tempVehicle(player, model, fwd, new alt.Vector3(0, 0, 0));
        } catch (err) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_MODEL_INVALID));
        }
    }
    ...
    private static setVehicleLiveryCommand(player: alt.Player, livery: number): void {
        ...
        triallife.vehicle.funcs.setLivery(vehicle, livery);
    }
    ...
    private static createSessionVehicle(player: alt.Player, model: string): void {
        ...
        try {
            vehicle = triallife.vehicle.funcs.sessionVehicle(player, model, player.pos, new alt.Vector3(0, 0, 0));
        } catch (err) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_MODEL_INVALID));
        }
        ...
        triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_CREATED));
    ...
    private static toggleVehicleNeonLightsCommand(player: alt.Player): void {
        ...
        triallife.vehicle.funcs.setNeonLightsEnabled(vehicle, lightsEnabled);
        triallife.vehicle.funcs.save(vehicle, vehicle.data);
    }

    @command(['setneonlights', 'snl'], LocaleController.get(LOCALE_KEYS.COMMAND_SET_VEH_NEON_LIGHTS, '/snl'), PERMISSIONS.ADMIN)
    private static setVehicleNeonLightsCommand(player: alt.Player, left: string, right: string, front: string, back: string): void {
        const vehicle = player.vehicle;

        if (!vehicle?.valid || vehicle.isTemporary) return;

        triallife.vehicle.funcs.setNeon(vehicle, {
            left: left === '1',
            right: right === '1',
            front: front === '1',
            back: back === '1',
    ...
    private static fullTuneVehicleCommand(player: alt.Player): void {
        ...
        if (vehicle.modKit == 0 && vehicle.modKitsCount > 0) triallife.vehicle.funcs.setModKit(vehicle, 1);

        if (vehicle.modKit == 0) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_HAS_NO_MOD_KIT));
            return;
        }
        ...
        for (let i = 0; i < 70; ++i) {
            ...
            if (maxId > 0) {
                triallife.vehicle.funcs.setMod(vehicle, i, maxId);
            }
        }
    ...
    private static addVehicleKeyCommand(player: alt.Player): void {
        ...
        triallife.vehicle.funcs.createKey(player, vehicle);
    }
