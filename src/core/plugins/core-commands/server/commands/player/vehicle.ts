...
class VehicleCommands {
    ...
    private static giveVehicleKeyCommand(player: alt.Player, id: string) {
        if (!player || !player.valid || id === null || id === undefined) {
            triallife.player.emit.notification(player, `/givevehkey [id]`);
            return;
        }
        ...
        triallife.player.emit.notification(player, `${LocaleController.get(LOCALE_KEYS.VEHICLE_KEY_GIVEN_TO)} ${target.data.name}.`);
    }
    ...
    private static handleSeatbeltCommand(player: alt.Player): void {
        ...
        if (currentState) {
            triallife.player.emit.sound2D(player, 'seatbelt_on', 0.75);
        } else {
            triallife.player.emit.sound2D(player, 'seatbelt_off', 0.75);
        }

        currentState
            ? triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.PLAYER_SEATBELT_ON))
            : triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.PLAYER_SEATBELT_OFF));
