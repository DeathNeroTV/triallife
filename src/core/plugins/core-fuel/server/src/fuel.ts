...
export class FuelSystem {
    ...
    static init() {
        ...
        triallife.vehicle.system.addCustomRule(VEHICLE_RULES.ENGINE, (player: alt.Player, vehicle: alt.Vehicle) => {
            if (!vehicle.driver || vehicle.driver.id !== player.id) {
                triallife.player.emit.notification(player, LOCALE_FUEL.VEHICLE_IS_NOT_DRIVER);
            ...
            if (!player.vehicle.engineOn && !FuelSystem.hasFuel(player.vehicle)) {
                triallife.player.emit.notification(player, LOCALE_FUEL.VEHICLE_NO_FUEL);
                return { status: false, response: LOCALE_FUEL.VEHICLE_NO_FUEL };
            }

            if (player.vehicle.isRefueling) {
                triallife.player.emit.notification(player, LOCALE_FUEL.VEHICLE_IS_BEING_REFUELED);
        ...
        triallife.injections.vehicle.add('vehicle-add-start', (document: IVehicle) => {
    ...
    static tick(vehicle: alt.Vehicle) {
        ...
        if (!vehicle.nextSave || Date.now() > vehicle.nextSave) {
            triallife.vehicle.funcs.save(vehicle, { fuel: vehicle.data.fuel });
