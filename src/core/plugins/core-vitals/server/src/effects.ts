...
export class InternalFunctions {
    ...
    static handleVitalsChange(player: alt.Player, item: Item, vitalsName: VITAL_NAMES) {
        ...
        triallife.player.inventory.notify(player, `+${item.data.amount} ${vitalsName}`);
        ...
        if (vitalsName === VITAL_NAMES.FOOD) {
            ...
            triallife.player.emit.objectAttach(player, attachedObject, 6000);
            triallife.player.emit.animation(
                player,
        ...
        if (vitalsName === VITAL_NAMES.WATER) {
            ...
            triallife.player.emit.objectAttach(player, attachedObject, 5000);
            triallife.player.emit.animation(
                player,
