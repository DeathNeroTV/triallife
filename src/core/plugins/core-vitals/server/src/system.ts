...
class InternalFunctions {
    ...
    static handlePing(player: alt.Player) {
        ...
        if (player.data.isDead) {
            ...
            triallife.player.emit.meta(player, VITAL_NAMES.FOOD, player.data[VITAL_NAMES.FOOD]);
            triallife.player.emit.meta(player, VITAL_NAMES.WATER, player.data[VITAL_NAMES.WATER]);
            return;
        }
...
export class VitalsSystem {
    ...
    static init() {
        ...
        PlayerEvents.on(TRIALLIFE_EVENTS_PLAYER.SELECTED_CHARACTER, (player) => {
    ...
    static forceUpdateVital(player: alt.Player, vitalName: VITAL_NAMES) {
        if (!player.data[vitalName]) {
            triallife.state.set(player, vitalName, 100);
        }

        triallife.player.emit.meta(player, vitalName, player.data[vitalName]);
    }
    ...
    static adjustVital(player: alt.Player, vitalName: VITAL_NAMES, value: number, isExact = false) {
        ...
        if (oldVitalValue !== player.data[vitalName]) {
            triallife.state.set(player, vitalName, oldVitalValue);
        }

        triallife.player.emit.meta(player, vitalName, oldVitalValue);
    }
