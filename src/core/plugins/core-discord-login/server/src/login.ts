...
export class LoginController {
     ...
     */
    static init() {
        PlayerEvents.on(TRIALLIFE_EVENTS_PLAYER.SELECTED_CHARACTER, LoginController.bindPlayerToID);
        ...
        if (!account) {
            ...
            if (!accountData) {
                ...
                if (player.discord.email && player.discord.email !== account.email) {
                    ...
                    await Database.updatePartialData(account._id, { email: player.discord.email }, Collections.Accounts);
                }
            }
        }
        if (account.banned) {
    ...
    static async tryDisconnect(player: alt.Player, reason: string): Promise<void> {
        ...
        if (typeof id === 'number') StorageView.removeStorageBinding(id);
        if (!player || !player.valid || !player.data || !player.data._id) return;
        if (player.isPushingVehicle) VehicleSystem.stopPush(player);
        playerConst.save.onTick(player);
        const players = alt.Player.all.filter((x) => x.id !== id && isFlagEnabled(x.accountData.permissionLevel, PERMISSIONS.MODERATOR | PERMISSIONS.ADMIN | PERMISSIONS.OWNER));
        for (const target of players) {
            ...
            data['factions'] = await Database.fetchAllData<Faction>(Collections.Factions);
