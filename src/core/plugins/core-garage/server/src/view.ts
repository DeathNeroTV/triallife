...
export class GarageFunctions {
    ...
    static async open(player: alt.Player, shopIndex: number | string) {
        ...
        // 2
        let playerVehicles = await triallife.player.get.allVehicles(player);
        ...
        const validVehicles = playerVehicles.filter((vehicle) => {
            ...
            // 7
            const existingVehicle = alt.Vehicle.all.find((x) => x.data && x.data._id && vehicle._id && x.data._id.toString() === vehicle._id.toString());
            ...
            if (triallife.vehicle.funcs.hasBeenSpawned(vehicle.id)) {
                return true;
            }
        ...
        if (validVehicles.length <= 0) {
            ...
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_NO_VEHICLES_IN_GARAGE));
            return;
        }
    ...
    static spawnVehicle(player: alt.Player, id: number) {
        ...
        if (triallife.vehicle.funcs.hasBeenSpawned(data.id)) {
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_ALREADY_SPAWNED));
        ...
        if (!openSpot) {
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_NO_PARKING_SPOTS));
            alt.emitClient(player, GARAGE_INTERACTIONS.CLOSE);
            return;
        }
        ...
        const newVehicle = triallife.vehicle.funcs.spawn(data, openSpot.position, openSpot.rotation);
    ...
    static despawnVehicle(player: alt.Player, id: number) {
        ...
        if (dist >= 10 && !GarageFunctions.isCloseToSpot(vehicle.pos, garage.parking)) {
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.VEHICLE_TOO_FAR));
            return;
        }

        // Set the garage index.
        vehicle.data.garageIndex = shopIndex;
        triallife.vehicle.funcs.save(vehicle, { garageIndex: vehicle.data.garageIndex });
