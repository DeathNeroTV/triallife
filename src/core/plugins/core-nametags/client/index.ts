...
class ClientNametags {
    ...
    static render() {
        ...
        for (let i = 0, n = alt.Player.all.length; i < n; i++) {
            ...
            if (player.getSyncedMeta('NoClipping')) continue;
            if (player.vehicle && alt.Player.local.vehicle !== player.vehicle) continue;
            ...
            if (!config.SHOW_NAME_INSTANTLY) {
                if (player.inVisionTime === null || (player.inVisionTime === undefined && isNaN(player.inVisionTime))) player.inVisionTime = Date.now() + 5000;
                if (Date.now() < player.inVisionTime) name = '';
            }
            ...
            if (name !== '') actualName += config.SHOW_NAME ? ` - ${name}` : ``;
