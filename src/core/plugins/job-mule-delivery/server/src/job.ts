    ...
    static async begin(player: alt.Player) {
        ...
        objectives.push({
            ...
            callbackOnStart: (player: alt.Player) => {
                ...
                triallife.player.emit.notification(player, `Get in the Mule`);
            },
        });
        ...
        for (let i = 0; i < randomPoints.length; i++) {
            ...
            objectives.push({
                ...
                callbackOnFinish: (player: alt.Player) => {
                    triallife.player.emit.notification(player, `You have dropped off the package`);
                },
            });
        }
        ...
        objectives.push({
            ...
            criteria: JobEnums.ObjectiveCriteria.FAIL_ON_JOB_VEHICLE_DESTROY | JobEnums.ObjectiveCriteria.IN_JOB_VEHICLE | JobEnums.ObjectiveCriteria.NO_DYING,
            callbackOnStart: (player: alt.Player) => {
                triallife.player.emit.notification(player, `Drive the vehicle back`);
            },
            callbackOnFinish: (player: alt.Player) => {
        ...
        const job = new Job();
        job.addVehicle(player, 'mule', openSpot.pos, openSpot.rot, new alt.RGBA(255, 255, 255, 255), new alt.RGBA(255, 255, 255, 255));
