...
class WeatherCommands {
    ...
    private static setWeather(player: alt.Player, weatherName: string): void {
        ...
        if (!weatherName) {
            triallife.player.emit.notification(player, `Must specify a weather name.`);
            return;
        }
        ...
        if (index <= -1) {
            triallife.player.emit.notification(player, `${weatherName} is not a valid weather type.`);
            return;
        }

        triallife.player.emit.notification(player, `${weatherName} is now overriding the entire weather system.`);
        triallife.player.emit.notification(player, `Use /clearweather to stop overriding the weather.`);
     ...
     */
    @command('clearweather', LocaleController.get(LOCALE_KEYS.COMMAND_CLEAR_WEATHER, '/clearweather'), PERMISSIONS.ADMIN)
    private static clearWeather(player: alt.Player) {
        ...
        triallife.player.emit.notification(player, `Weather override is now disabled.`);
    }
}
