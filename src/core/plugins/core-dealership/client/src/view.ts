...
class InternalFunctions implements ViewModel {
    ...
    static async ready() {
        ...
        if (alt.Player.local.meta.banks.find((x) => x.type === 'private'))
            view.emit(DEALERSHIP_WEBVIEW_EVENTS.SET_BANK, alt.Player.local.meta.cash + alt.Player.local.meta.banks.find((x) => x.type === 'private').amount);
        else view.emit(DEALERSHIP_WEBVIEW_EVENTS.SET_BANK, alt.Player.local.meta.cash);
    }
    ...
    static async preview(vehicle: VehicleInfo, color: number) {
        ...
        // Create Vehicle
        vehicleIdentifier = native.createVehicle(model, dealership.vehiclePosition.x, dealership.vehiclePosition.y, dealership.vehiclePosition.z, 0, false, false, false);

        native.setEntityInvincible(vehicleIdentifier, true);
        native.setEntityCoordsNoOffset(vehicleIdentifier, dealership.vehiclePosition.x, dealership.vehiclePosition.y, dealership.vehiclePosition.z + 0.2, false, false, false);
    ...
    static generateCameraPoints(vehicle: number): Array<alt.Vector3> {
        ...
        for (let i = 0; i < offsetCalculations.length; i++) {
            const calc = native.getOffsetFromEntityInWorldCoords(vehicle, offsetCalculations[i].x, offsetCalculations[i].y, 1);
