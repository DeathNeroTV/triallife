<template>
    <div class="eyebrows">
        ...
        <div class="split split-full-width">
            <Button class="eye-btn"  @click="$emit('decrement-index', 'eyeIndex', eyebrows.length - 1)">
                <Icon icon="icon-chevron-left" :size="24" />
            </Button>
            <div class="text-sm-overline white--text mb-2 header-bold">
                {{ eyebrows[currentIndex] }}
            </div>
            <Button class="eye-btn"  @click="$emit('increment-index', 'eyeIndex', eyebrows.length - 1)">
        ...
        <div class="split split-full-width">
            <Button class="eye-btn"  @click="$emit('decrement-index', 'eyeOpacity', 1, 0.1)">
                <Icon icon="icon-chevron-left" :size="24" />
            </Button>
            <div class="text-sm-overline boldest white--text mb-2 header-bold">
                {{ opacity.toFixed(2) }}
            </div>
            <Button class="eye-btn"  @click="$emit('increment-index', 'eyeOpacity', 1, 0.1)">
                <Icon icon="icon-chevron-right" :size="24" />
            </Button>
        </div>
    </div>
