<template>
    <div class="makeup">
        ...
        <div class="split split-full-width">
            <Button class="makeup-btn"  @click="$emit('decrement-index', 'beardIndex', makeup.length - 1)">
                <Icon icon="icon-chevron-left" :size="24" />
            </Button>
            <div class="text-sm-overline white--text mb-2 header-bold">
                {{ makeup[currentIndex] }}
            </div>
            <Button class="makeup-btn"  @click="$emit('increment-index', 'beardIndex', makeup.length - 1)">
        ...
        <div class="split split-full-width">
            <Button class="makeup-btn"  @click="$emit('decrement-index', 'beardOpacity', 1, 0.1)">
                <Icon icon="icon-chevron-left" :size="24" />
            </Button>
            <div class="text-sm-overline boldest white--text mb-2 header-bold">
                {{ opacity.toFixed(2) }}
            </div>
            <Button class="makeup-btn"  @click="$emit('increment-index', 'beardOpacity', 1, 0.1)">
                <Icon icon="icon-chevron-right" :size="24" />
            </Button>
        </div>
    </div>
