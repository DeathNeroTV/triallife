...
export class InternalFunctions {
    ...
    static init() {
        ...
        for (const pos of BARBER_SHOP_LOCATIONS) {
            ...
            triallife.controllers.marker.append({ pos, color: new alt.RGBA(0, 255, 0, 100), type: 1 });
            triallife.controllers.interaction.add({ position: pos, callback: BarbershopView.open, isPlayerOnly: true });
        }
    }
    ...
    static handleDisconnect(hairDresser: alt.Player) {
        const hairDresserID = triallife.systems.identifier.getIdByStrategy(hairDresser);
        ...
        triallife.player.set.frozen(customer, false);
        triallife.player.emit.notification(customer, BARBER_SHOP_LOCALE.HAIR_DRESSER_DISCONNECTED);
    }
}
...
export class BarbershopView {
    ...
    static async save(hairDresser: alt.Player, data: BarbershopData) {
        try {
            ...
            const customer = triallife.systems.identifier.getPlayer(sessions[hairDresserID]);
            if (!customer || !customer.valid) {
                return;
            }
            ...
            if (customer.data.appearance) {
                ...
                appearance.hairOverlay = data.hairFullName ? hairOverlayInfo[data.hairFullName] : { collection: '', overlay: '' };
                ...
                await triallife.state.set(customer, 'appearance', appearance);
            }
    ...
    static close(hairDresser: alt.Player, doNotSync = false) {
        ...
        triallife.player.sync.appearance(customer, customer.data.appearance as Appearance);
        triallife.player.sync.equipment(customer, customer.data.equipment as Item<ClothingComponent>[], customer.data.appearance.sex === 1);
    }
    ...
    static offer(hairDresser: alt.Player, customer: alt.Player) {
        const hairDresserID = triallife.systems.identifier.getIdByStrategy(hairDresser);
        const customerID = triallife.systems.identifier.getIdByStrategy(customer);
        ...
        triallife.player.emit.notification(hairDresser, `${BARBER_SHOP_LOCALE.HAVE_OFFERED} ${customer.data.name} ${BARBER_SHOP_LOCALE.AS_HAIRCUT_SESSION}`);
        triallife.player.emit.notification(customer, `${BARBER_SHOP_LOCALE.HAVE_BEEN_OFFERED_A_HAIRCUT_SESSION_BY} ${hairDresser.data.name}. /hairaccept ${hairDresserID}`);
    }
    ...
    static accept(customer: alt.Player, _hairDresserID: number): void {
        const hairDresser = triallife.systems.identifier.getPlayer(_hairDresserID);

        if (!hairDresser || !hairDresser.valid) {
            triallife.player.emit.notification(customer, BARBER_SHOP_LOCALE.CANNOT_FIND_THE_HAIRDRESSER);
            return;
        }
        ...
        if (offers[_hairDresserID].toString() !== customerID.toString()) {
            triallife.player.emit.notification(customer, BARBER_SHOP_LOCALE.HAIRDRESSER_IS_WITH_CUSTOMER);
            return;
        }
    ...
    static async open(hairDresser: alt.Player) {
        ...
        if (sessions[hairDresserID] === undefined) {
            ...
            customer = triallife.systems.identifier.getPlayer(sessions[hairDresserID]);
        }
        ...
        await triallife.player.inventory.unequip(customer, EQUIPMENT_TYPE.HAT);
