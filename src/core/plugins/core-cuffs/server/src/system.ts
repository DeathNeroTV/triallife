...
export class CuffSystem {
    ...
    static async init() {
        ...
        PlayerEvents.on(TRIALLIFE_EVENTS_PLAYER.DIED, CuffSystem.handleDeath);
    }
    ...
    static async cuff(cuffer: alt.Player, item: Item, slot: number, inventoryType: INVENTORY_TYPE) {
        ...
        if (!target) {
            triallife.player.emit.notification(cuffer, `Could not find a target player.`);
            return;
        }
        ...
        if (target.isCuffed) {
            triallife.player.emit.notification(cuffer, `Target is already cuffed.`);
            return;
        }

        if (inventoryType === INVENTORY_TYPE.TOOLBAR) {
            if (!triallife.player.inventory.toolbarRemove(cuffer, item.slot)) {
                triallife.player.emit.notification(cuffer, `Could not find cuffs`);
                return;
            }

            triallife.state.set(cuffer, 'toolbar', cuffer.data.toolbar);
        }

        if (inventoryType === INVENTORY_TYPE.INVENTORY) {
            if (!triallife.player.inventory.inventoryRemove(cuffer, item.slot)) {
                triallife.player.emit.notification(cuffer, `Could not find cuffs`);
                return;
            }
        }
        ...
        const invRef = triallife.player.inventory.getFreeInventorySlot(cuffer);

        if (!invRef) {
            triallife.player.emit.notification(cuffer, `No room in inventory`);
            return;
        }

        // Add the Key Item to the Cuffer
        triallife.player.inventory.inventoryAdd(cuffer, keyItem, invRef.slot);

        // Save Inventory & Synchronize Changes
        triallife.state.setBulk(cuffer, { inventory: cuffer.data.inventory, toolbar: cuffer.data.toolbar });
        triallife.player.sync.inventory(cuffer);
    ...
    static detach(cuffer: alt.Player) {
        ...
        triallife.player.emit.animation(target, 'mp_arresting', 'idle', 49);
    }
    ...
    static attach(cuffer: alt.Player) {
        ...
        triallife.player.emit.animation(target, 'mp_arresting', 'idle', 49);
        target.attachTo(cuffer, 0, 0, { x: 0.2, y: 1, z: 0 } as alt.Vector3, { x: 0, y: 0, z: 0 } as alt.Vector3, false, false);
    }
    ...
    static putIntoVehicle(cuffer: alt.Player) {
        ...
        if (!vehicle) {
            triallife.player.emit.notification(cuffer, `Could not find vehicle close enough.`);
            return;
        }

        if (!VehicleFuncs.hasOwnership(cuffer, vehicle)) {
            triallife.player.emit.notification(cuffer, `This is not your vehicle.`);
            return;
        }
    ...
    static removeFromVehicle(cuffer: alt.Player) {
        ...
        if (target.vehicle.id !== vehicle.id) {
            triallife.player.emit.notification(cuffer, `Cuffed player is not in this vehicle.`);
            return;
        }

        triallife.player.safe.setPosition(target, cuffer.pos.x, cuffer.pos.y, cuffer.pos.z);
        CuffSystem.attach(cuffer);
    }
    ...
    static cuffMenu(cuffer: alt.Player) {
        ...
        // Emit the wheel menu
        triallife.player.emit.wheelMenu(cuffer, `${cuffed.data.name}`, options);
    }
}
