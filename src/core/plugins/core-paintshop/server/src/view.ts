...
class InternalFunctions {
    ...
    static updatePaint(vehicle: alt.Vehicle) {
        ...
        if (vehicle.data.tuning.primaryColor && !vehicle.data.tuning.secondaryColor) vehicle.data.tuning.secondaryColor = vehicle.data.tuning.primaryColor;

        if (vehicle.data.tuning.secondaryColor && !vehicle.data.tuning.primaryColor) vehicle.data.tuning.primaryColor = vehicle.data.tuning.secondaryColor;

        if (vehicle.data.tuning.primaryColor) {
            if (typeof vehicle.data.tuning.primaryColor == 'object')
                vehicle.customPrimaryColor = new alt.RGBA(vehicle.data.tuning.primaryColor.r, vehicle.data.tuning.primaryColor.g, vehicle.data.tuning.primaryColor.b);
        ...
        if (vehicle.data.tuning.secondaryColor) {
            if (typeof vehicle.data.tuning.secondaryColor == 'object')
                vehicle.customSecondaryColor = new alt.RGBA(vehicle.data.tuning.secondaryColor.r, vehicle.data.tuning.secondaryColor.g, vehicle.data.tuning.secondaryColor.b);
            else vehicle.secondaryColor = vehicle.data.tuning.secondaryColor;
        }
    }

    static previewPaint(player: alt.Player, color: number | RGBA, color2: number | RGBA, finish1: number, finish2: number, pearl: number = -1) {
        if (!inShop[player.id]) {
            return;
        }
...
export class PaintShopView {
    ...
    static register(shop: IPaintShop): string {
        ...
        const polygon = new PolygonShape(shop.vertices[0].z - 2.5, shop.vertices[0].z + 2.5, shop.vertices, true, false);
    ...
    static enter(polygon: PolygonShape, player: alt.Player) {
        ...
        if (!player.vehicle) {
            triallife.player.emit.notification(player, PAINTSHOP_LOCALE.MUST_BE_IN_A_VEHICLE);
            return;
        }

        if (!player.vehicle.data || player.vehicle.isTemporary) {
            triallife.player.emit.notification(player, PAINTSHOP_LOCALE.CANNOT_BE_MODIFIED);
            return;
        }
        ...
        triallife.player.emit.interactionAdd(player, {
            keyPress: 'E',
            description: PAINTSHOP_LOCALE.OPEN_MENU,
            uid: polygon.uid,
        });
        triallife.player.emit.interactionTemporary(player, Paintshop_View_Events.OPEN);
    }
    ...
    static leave(polygon: PolygonShape, player: alt.Player) {
        ...
        delete inShop[player.id];
        triallife.player.emit.interactionRemove(player, polygon.uid);
        triallife.player.emit.interactionTemporary(player, null);
    }
     ...
     * @returns Nothing.
     */
    static purchase(player: alt.Player, color: RGBA | number, color2: RGBA | number, finish1: number, finish2: number, pearl: number) {
        if (!player.vehicle || player.vehicle.driver !== player) {
            return;
        }
