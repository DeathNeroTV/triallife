...
class InternalFunctions implements ViewModel {
    ...
    static async purchase(color1: RGB | number, color2: RGB | number, isCustom = false, finish1: number, finish2: number, pearl: number) {
    ...
    static async update(primary: RGB | number, secondary: RGB | number, isCustom = false, finish1: number, finish2: number, pearl: number) {
    ...
    static generateCameraPoints(): Array<Vector3> {
        ...
        for (let i = 0; i < offsetCalculations.length; i++) {
            const calc = native.getOffsetFromEntityInWorldCoords(alt.Player.local.vehicle.scriptID, offsetCalculations[i].x, offsetCalculations[i].y, 1);
