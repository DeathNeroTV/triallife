    ...
    async init() {
        ...
        for (let i = 0; i < accounts.length; i++) {
            if (accounts[i].id === null || accounts[i].id === undefined) {
                ...
                await Database.updatePartialData(accounts[i]._id.toString(), { id: nextIdentifier }, Collections.Accounts);
 ...
 */
function override<Key extends keyof typeof AccountSystemRef>(functionName: Key, callback: typeof AccountSystemRef[Key]): void {
