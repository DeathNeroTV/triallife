...
export class Job {
     ...
     * @memberof Job
     */
    addVehicle(player: alt.Player, model: string, pos: Vector3, rot: Vector3, color1?: alt.RGBA, color2?: alt.RGBA): alt.Vehicle {
        const veh = triallife.vehicle.funcs.sessionVehicle(player, model, pos, rot);
    ...
    async checkObjective(): Promise<boolean> {
        ...
        if (objective.particle) {
            triallife.player.emit.particle(this.player, objective.particle, true);
        }
    ...
    quit(reason: string) {
        ...
        if (this.player.valid) {
            ...
            triallife.player.emit.notification(this.player, reason);
        }
    ...
    private verifyCriteria(objective: Objective): boolean {
        ...
        if (isFlagEnabled(objective.criteria, JobEnums.ObjectiveCriteria.NO_WEAPON)) {
            if (triallife.player.inventory.hasWeapon(this.player)) {
                return false;
            }
        }
    ...
    goToNextObjective() {
        ...
        if (this.objectives.length <= 0) {
            this.removeAllVehicles();
            triallife.player.emit.notification(this.player, `Job Completed`);
            alt.emitClient(this.player, JobEnums.ObjectiveEvents.JOB_SYNC, null);
            return;
        }
    ...
    private tryAnimation() {
        ...
        setTimeout(() => {
            ...
            alt.nextTick(() => {
                triallife.player.emit.animation(this.player, objective.animation.dict, objective.animation.name, objective.animation.flags, objective.animation.duration);
            });
        }, delay);
    }
    ...
    private tryAttach() {
        ...
        triallife.player.emit.objectAttach(this.player, objectToAttach, objective.attachable.duration);
    }
    ...
    removeAttachable() {
        ...
        triallife.player.emit.objectRemove(this.player, objective.attachable.uid);
    }
