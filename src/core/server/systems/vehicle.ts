...
export class VehicleSystem {
     ...
     * @memberof VehicleSystem
     */
    static addCustomRule(ruleType: VEHICLE_RULES, callback: (player: alt.Player, vehicle: alt.Vehicle, data?: IVehicleRuleData) => IResponse) {
        if (!rules[ruleType]) {
    ...
    static toggleDoor(player: alt.Player, doorNumber: number) {
        const vehicle = player.vehicle ? player.vehicle : getClosestEntity<alt.Vehicle>(player.pos, player.rot, alt.Vehicle.all, 5);
    ...
    static toggleLock(player: alt.Player): void {
        ...
        vehicle.lockState = (vehicle.lockState as number) === VEHICLE_LOCK_STATE.LOCKED ? VEHICLE_LOCK_STATE.UNLOCKED : VEHICLE_LOCK_STATE.LOCKED;
        ...
        if (!player.vehicle) {
            playerConst.emit.animation(player, `anim@mp_player_intmenu@key_fob@`, 'fob_click_fp', ANIMATION_FLAGS.UPPERBODY_ONLY | ANIMATION_FLAGS.ENABLE_PLAYER_CONTROL, -1);
        }
    ...
    static async storage(player: alt.Player, vehicle: alt.Vehicle) {
        ...
        const vehicleInfo = VehicleData.find((x) => x.name.toLocaleLowerCase() === vehicle.data.model.toLocaleLowerCase());
