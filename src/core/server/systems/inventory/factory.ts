...
async function init() {
    ...
    await triallife.database.funcs.createCollection(COLLECTION_NAME);

    // Read all base items from database.
    baseItems = await triallife.database.funcs.fetchAllData<BaseItem>(COLLECTION_NAME);
...
async function add(baseItem: BaseItem): Promise<boolean> {
    ...
    const newBaseItem = await triallife.database.funcs.insertData(baseItem, COLLECTION_NAME, true);
...
async function update(baseItem: Omit<BaseItem, 'version'>): Promise<boolean> {
    ...
    const baseItemCopy = triallife.utility.deepCloneObject<BaseItem>(baseItems[index]);
    ...
    return await triallife.database.funcs.updatePartialData(baseItems[index]._id, baseItemCopy, COLLECTION_NAME);
}
