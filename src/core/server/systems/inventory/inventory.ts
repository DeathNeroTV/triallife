...
async function add(
    ...
    if (!stack) {
        ...
        triallife.player.sync.inventory(player as alt.Player);
 ...
 * @return {Promise<number>}
 */
async function sub(player: CharacterInventory | Pick<alt.Player, 'data'>, baseName: string, quantityToRemove: number, strict: boolean): Promise<number> {
    if (typeof player.data.inventory === 'undefined') {
        player.data.inventory = [];
 ...
 */
async function remove<T = StoredItem>(player: CharacterInventory | Pick<alt.Player, 'data'>, slot: number): Promise<Item<T> | false> {
    if (typeof player.data.inventory === 'undefined') {
        player.data.inventory = [];
        return false;
    }
 ...
 * @return {boolean}
 */
function hasDataKeys<T = StoredItem>(player: CharacterInventory | Pick<alt.Player, 'data'>, slot: number, keys: Array<string>): boolean {
    if (typeof player.data.inventory === 'undefined') {
        player.data.inventory = [];
        return false;
    }
