 ...
 */
function find<T = Object>(player: CharacterInventory<T> | Pick<alt.Player, 'data'>, type: ItemSlot, base: string): Item<T> | undefined {
 ...
 */
async function updateData<Data = Object>(player: CharacterInventory | Pick<alt.Player, 'data'>, type: ItemSlot, slot: number, data: Array<string>);
 ...
 */
async function updateData<Data = Object>(player: CharacterInventory | Pick<alt.Player, 'data'>, type: ItemSlot, slot: number, data: Data): Promise<boolean> {
    if (!player.data.hasOwnProperty(type) && !Array.isArray(player.data[type])) {
        player.data[type] = [];
        return false;
    }
 ...
 * @return {Promise<boolean>}
 */
async function update(player: CharacterInventory | Pick<alt.Player, 'data'>, type: ItemSlot, slot: number, data: Partial<StoredItem>): Promise<boolean> {
    if (!player.data.hasOwnProperty(type) && !Array.isArray(player.data[type])) {
        player.data[type] = [];
        return false;
    }
 ...
 */
function queryData(player: CharacterInventory | Pick<alt.Player, 'data'>, type: ItemSlot, key: string, value: any): Item | undefined {
    if (!player.data.hasOwnProperty(type) && !Array.isArray(player.data[type])) {
        player.data[type] = [];
        return undefined;
    }
 ...
 */
function findAvailableSlot(player: CharacterInventory | Pick<alt.Player, 'data'>, type: ItemSlot, maxSlotCount: number): number | undefined {
    if (!player.data.hasOwnProperty(type) && !Array.isArray(player.data[type])) {
        player.data[type] = [];
        return 0;
    }
