...
const Setter = {
    ...
    async firstConnect(player: alt.Player): Promise<void> {
        ...
        if (process.env.triallife_READY === 'false') {
            player.kick('Still warming up...');
            return;
        }

        const vueDefaultPath = ConfigUtil.getVueDebugMode() ? ConfigUtil.getViteServer() : `http://assets/webviews/index.html`;
    ...
    respawned(player: alt.Player, position: alt.IVector3): void {
        ...
        PlayerEvents.trigger(TRIALLIFE_EVENTS_PLAYER.SPAWNED, player, position);
    },
    ...
    wantedLevel(player: alt.Player, stars: number) {
        ...
        player.wanted = stars;
        triallife.state.set(player, 'wanted', stars);
...
function override<Key extends keyof typeof Setter>(functionName: Key, callback: typeof Setter[Key]): void {
    if (typeof exports[functionName] === 'undefined') {
        alt.logError(`triallife.player.setter does not provide an export named ${functionName}`);
    }
