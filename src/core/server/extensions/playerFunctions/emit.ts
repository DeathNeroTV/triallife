...
const Emit = {
    startAlarm(player: alt.Player, name: string): void {
        if (player.data.isDead) {
            alt.logWarning(`[triallife] Cannot play alarm ${name} while player is dead.`);
            return;
        }
    ...
    animation(player: alt.Player, dictionary: string, name: string, flags: ANIMATION_FLAGS, duration: number = -1): void {
        if (player.data.isDead) {
            alt.logWarning(`[triallife] Cannot play ${dictionary}@${name} while player is dead.`);
            return;
        }
...
function override<Key extends keyof typeof Emit>(functionName: Key, callback: typeof Emit[Key]): void {
    if (typeof exports[functionName] === 'undefined') {
        alt.logError(`triallife.player.emit does not provide an export named ${functionName}`);
    }
