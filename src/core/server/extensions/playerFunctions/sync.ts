...
const Sync = {
    async currencyData(player: alt.Player): Promise<void> {
        const banks = await triallife.player.currency.getAllBankAccountsPlayer(player);
        const cash = player.data.cash;
        emit.meta(player, 'cash', cash);
        emit.meta(player, 'banks', banks);
        triallife.state.setBulk(player, { banks, cash });
    },

    inventory(playerRef: alt.Player): void {
        const player = playerRef as ReadOnlyPlayer;
        if (!player.data.inventory) triallife.state.set(player, 'inventory', []);
        if (!player.data.toolbar) triallife.state.set(player, 'toolbar', []);
        if (!player.data.equipment) triallife.state.set(player, 'equipment', []);
    ...
    appearance(player: alt.Player, appearance: Appearance) {
        if (appearance.sex === 0) player.model = 'mp_f_freemode_01';
        ...
        if (appearance.hairOverlay) decorationsToSync.push(appearance.hairOverlay);
        if (decorationsToSync.length >= 1) alt.emitClient(player, SYSTEM_EVENTS.SET_PLAYER_DECORATIONS, decorationsToSync);
        // Hair - Supports DLC
        if (typeof appearance.hairDlc === 'undefined' || appearance.hairDlc === 0) player.setClothes(2, appearance.hair, 0, 0);
        else player.setDlcClothes(appearance.hairDlc, 2, appearance.hair, 0, 0);
        ...
        for (let i = 0; i < clothingComponents.length; i++) {
            ...
            for (let index = 0; index < component.drawables.length; index++) {
                ...
                if (component.dlcHashes && component.dlcHashes.length >= 1 && component.dlcHashes[index] !== 0) {
                    ...
                    if (typeof dlc === 'string') dlc = alt.hash(dlc);
                    if (component.isProp) {
    ...
    playTime(playerRef: alt.Player): void {
        const player = playerRef as ReadOnlyPlayer;
        triallife.state.set(player, 'hours', (player.data?.hours ?? 0) + 0.0166666666666667, true);
    },
};
...
function override<Key extends keyof typeof Sync>(functionName: Key, callback: typeof Sync[Key]): void {
    if (typeof exports[functionName] === 'undefined') {
        alt.logError(`triallife.player.sync does not provide an export named ${functionName}`);
    }
