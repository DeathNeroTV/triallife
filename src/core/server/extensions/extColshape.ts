...
export class InteractionShape extends alt.ColshapeCylinder {
    ...
    constructor(interaction: Required<Interaction>) {
        super(interaction.position.x, interaction.position.y, interaction.position.z, interaction.range, interaction.height ? interaction.height : DEFAULT_INTERACTION_HEIGHT);
...
export class PolygonShape extends alt.ColshapePolygon {
     ...
     */
    constructor(minZ: number, maxZ: number, vertices: Vector2[] | Vector3[], isPlayerOnly: boolean, isVehicleOnly: boolean, debug = false) {
