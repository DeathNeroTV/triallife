...
const TEMPORARY_VEHICLE = Vehicle_Behavior.NO_KEY_TO_LOCK | Vehicle_Behavior.NO_KEY_TO_START | Vehicle_Behavior.UNLIMITED_FUEL | Vehicle_Behavior.NO_SAVE;
...
export default class VehicleFuncs {
    ...
    static async spawnPlayerVehicles(vehicles: Array<IVehicle>) {
        ...
        for (const vehicle of vehicles) {
            ...
            const existingVehicle = alt.Vehicle.all.find((veh) => veh && veh.valid && veh.data && veh.data._id.toString() === vehicle._id.toString());
    ...
    static sessionVehicle(player: alt.Player, model: string, pos: alt.IVector3, rot: alt.IVector3) {
        ...
        vehicle.behavior = Vehicle_Behavior.NEED_KEY_TO_START | Vehicle_Behavior.NO_SAVE | Vehicle_Behavior.UNLIMITED_FUEL;
     ...
     * @memberof VehicleFuncs
     */
    static tempVehicle(player: alt.Player, model: string, pos: alt.IVector3, rot: alt.IVector3, doNotDelete: boolean = false): alt.Vehicle {
        const vehicle = new alt.Vehicle(model, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z);
        vehicle.player_id = player.id;
    ...
    static async save(vehicle: alt.Vehicle, dataObject: Partial<IVehicle>): Promise<boolean> {
        ...
        const saveInjections = Injections.get<(vehicle: alt.Vehicle) => { [key: string]: any }>(VehicleInjectionNames.SAVE);
    ...
    static hasOwnership(player: alt.Player, vehicle: alt.Vehicle): boolean {
        ...
        if (vehicle.data.owner === player.data._id) {
            return true;
        }
     ...
     * @memberof VehicleFuncs
     */
    static async createKey(player: alt.Player, vehicle: alt.Vehicle | IVehicle, doNotAddToInventory: boolean = false): Promise<VehicleKeyItem | null> {
        ...
        if (!doNotAddToInventory) {
            const inventory = triallife.player.inventory.getFreeInventorySlot(player);
            if (!inventory) {
                triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.INVENTORY_IS_FULL));
                return null;
            }

            if (!triallife.player.inventory.inventoryAdd(player, item, inventory.slot)) {
                triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.INVENTORY_IS_FULL));
                return null;
            }

            await triallife.state.set(player, 'inventory', player.data.inventory, true);
    ...
    static applyVehicleTuning(vehicle: alt.Vehicle): void {
        ...
        if (typeof data.primaryColor === 'number') {
            ...
            vehicle.customPrimaryColor = new alt.RGBA(data.primaryColor.r, data.primaryColor.g, data.primaryColor.b, data.primaryColor.a);
        }

        if (typeof data.secondaryColor === 'number') {
            vehicle.secondaryColor = data.secondaryColor;
        } else {
            vehicle.customSecondaryColor = new alt.RGBA(data.secondaryColor.r, data.secondaryColor.g, data.secondaryColor.b, data.secondaryColor.a);
        }
        ...
        if (data.tireSmokeColor) {
            vehicle.tireSmokeColor = new alt.RGBA(data.tireSmokeColor.r, data.tireSmokeColor.g, data.tireSmokeColor.b, data.tireSmokeColor.a);
        }
    ...
    private static convertOldTuningData(vehicle: IVehicle): IVehicle {
        ...
        if (vehicle.color) {
            ...
            else vehicle.tuning.primaryColor = new alt.RGBA(vehicle.color.r, vehicle.color.g, vehicle.color.b, vehicle.color.a);
        ...
        if (vehicle.color2) {
            ...
            else vehicle.tuning.secondaryColor = new alt.RGBA(vehicle.color2.r, vehicle.color2.g, vehicle.color2.b, vehicle.color2.a);
        ...
        if (hasChanged)
            triallife.database.funcs.removePartialData(
                vehicle._id,
                { color: true, color2: true, finish1: true, finish2: true, pearl: true },
                triallife.database.collections.Vehicles,
            );
