...
export class triallifeScreenshot {
    /**
    ...
    static async takeScreenshot(player: alt.Player): Promise<string | null> {
        ...
        return new Promise((resolve: Function) => {
            ...
            const interval = alt.setInterval(() => {
                ...
                alt.clearInterval(interval);
                const fullData = triallifeBuffer.fromBuffer(pendingScreenshots[player.id].data);
                delete pendingScreenshots[player.id];
...
alt.onClient(SYSTEM_EVENTS.SCREENSHOT_POPULATE_DATA, triallifeScreenshot.buildData);
