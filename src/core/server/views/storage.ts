...
export class StorageView {
    ...
    static async open(player: alt.Player, storage_id: string, name: string, entity?: alt.Entity): Promise<void> {
        ...
        if (!storage) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.STORAGE_NOT_AVAILABLE));
        ...
        if (StorageSystem.isRestricted(storage_id)) {
            triallife.player.emit.notification(player, LocaleController.get(LOCALE_KEYS.STORAGE_IN_USE));
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            return;
        }
     ...
     * @memberof StorageView
     */
    static addMovementRule(ruleType: STORAGE_MOVE_RULES, callback: (player: alt.Player, itemBeingMoved: Item, amount: number) => IResponse) {
    ...
    static async moveFromStorage(player: alt.Player, id: string, index: number, amount: number) {
        ...
        if (dist > DEFAULT_MAX_STORAGE_DIST) {
            ...
            StorageSystem.setRestricted(id, false);
            triallife.player.emit.soundFrontend(player, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS');
            triallife.player.emit.notification(player, `Too Far From Storage`);
            alt.emitClient(player, View_Events_Storage.Close);
            return;
        }
        ...
        const invInfo = triallife.player.inventory.getFreeInventorySlot(player);
        if (invIndex <= -1 || !canStack) {
            ...
            if (!triallife.player.inventory.inventoryAdd(player, itemClone, invInfo.slot)) {
        ...
        if (!itemClone.maxStack || amount + player.data.inventory[invIndex].quantity < storageCache[player.id].items[index].maxStack) {
    ...
    static async moveFromPlayer(player: alt.Player, id: string, index: number, amount: number) {
        ...
        if (amount === itemClone.quantity) {
            if (!triallife.player.inventory.inventoryRemove(player, player.data.inventory[index].slot)) {
                alt.emitClient(player, View_Events_Storage.Refresh, player.data.inventory, storageCache[player.id].items);
                return;
            }
        } else {
