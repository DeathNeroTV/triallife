...
export class InventoryView {
     ...
     */
    static addRule(rule: INVENTORY_RULES, callback: (player: alt.Player, item: Item, selectedSlot: number, endSlot: number, rule: string) => boolean): boolean {
        if (!ITEM_RULES[rule]) {
            return false;
        }
     ...
     * @memberof InventoryView
     */
    static async verifyRules(player: alt.Player, rule: INVENTORY_RULES, item: Item, selectedSlot: number, endSlot: number): Promise<boolean> {
     ...
     * @memberof InventoryView
     */
    static getInventoryRule(selectData: CategoryData, endData: CategoryData): { selected: INVENTORY_RULES; end: INVENTORY_RULES } | null {
        // Inventory Rules
     ...
     * @memberof InventoryView
     */
    static async processItemMovement(player: alt.Player, selectedSlot: string, endSlot: string, hash: string | null): Promise<void> {
        ...
        if (selectData.abbrv !== endData.abbrv) {
            ...
            const selectedResult = await InventoryView.verifyRules(player, rules.selected, itemClone, selectSlotIndex, endSlotIndex);
        ...
        if (endData.emptyCheck && !endData.emptyCheck(player, endSlotIndex)) {
            ...
            if (selectData.abbrv !== endData.abbrv) {
                ...
                const rules = InventoryView.getInventoryRule(selectData, endData);
                const selectedResult = await InventoryView.verifyRules(player, rules.selected, endItemClone, endSlotIndex, selectSlotIndex);
        ...
        stateConst.setBulk(player, { [selectData.name]: player.data[selectData.name], [endData.name]: player.data[endData.name] }, true);
    ...
    static async handleDropGround(player: alt.Player, selectedSlot: string) {
        ...
        if (isFlagEnabled(itemClone.behavior, ITEM_TYPE.DESTROY_ON_DROP)) {
            ...
            playerConst.emit.notification(player, LocaleController.get(LOCALE_KEYS.ITEM_WAS_DESTROYED_ON_DROP, itemClone.name));
            return;
        }
        ...
        playerConst.emit.animation(player, 'random@mugging4', 'pickup_low', 33, 1200);
        PlayerEvents.trigger(TRIALLIFE_EVENTS_PLAYER.DROPPED_ITEM, player, itemClone);
    }
    ...
    static async handlePickupGround(player: alt.Player, endData: CategoryData, endSlotIndex: number, hash: string | null) {
        if (player.vehicle) {
            playerConst.sync.inventory(player);
            return;
        }
