...
export class PushVehicle {
    ...
    static pushing() {
        ...
        const isPlayingAnim = native.isEntityPlayingAnim(alt.Player.local.scriptID, 'missfinale_c2ig_11', 'pushcar_offcliff_m', 3);
        ...
        if (native.isDisabledControlPressed(0, 32)) {
            if (!isPlayingAnim) {
                playAnimation('missfinale_c2ig_11', 'pushcar_offcliff_m', ANIMATION_FLAGS.REPEAT | ANIMATION_FLAGS.ENABLE_PLAYER_CONTROL);
            }
