...
export class TriallifeEvents {
    static init() {
        ...
        interval = Timer.createInterval(TriallifeEvents.tick, 250, 'triallife-events.ts');
    }
    ...
    static async updateWaypoint() {
        ...
        if (!native.doesBlipExist(waypoint)) {
            ...
            if (lastWaypointData !== null) {
                ...
                alt.emitServer(TRIALLIFE_EVENTS_PLAYER_CLIENT.WAYPOINT, null);
            }
        ...
        alt.emitServer(TRIALLIFE_EVENTS_PLAYER_CLIENT.WAYPOINT, foundWaypoint);
        native.requestCollisionAtCoord(alt.Player.local.pos.x, alt.Player.local.pos.y, alt.Player.local.pos.z);
        native.clearFocus();
        isUpdatingWaypoint = false;
    }
    ...
    static tick() {
        ...
        if (!isUpdatingWaypoint) {
            TriallifeEvents.updateWaypoint();
        }
    }
}

alt.onServer(SYSTEM_EVENTS.TICKS_START, TriallifeEvents.init);
