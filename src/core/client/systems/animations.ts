 ...
 */
export async function playAnimation(dict: string, name: string, flags: ANIMATION_FLAGS = ANIMATION_FLAGS.CANCELABLE, duration: number = -1): Promise<void> {
 ...
 * @return {*}
 */
export async function playPedAnimation(scriptID: number, dict: string, name: string, flags: ANIMATION_FLAGS = ANIMATION_FLAGS.CANCELABLE, duration: number = -1) {
    const isReadyToPlay = await loadAnimation(dict);
    if (!isReadyToPlay) {
        return;
    }
