...
export class WebViewController {
    ...
    /**
     * Trigger this to hide/show all overlays like HUD, etc.
     * @static
     * @param {boolean} value
     * @memberof WebViewController
     */
     ...
     * @memberof WebViewController
     */
    static async openPages(pageOrPages: Array<string> | string, hideOverlays: boolean = true, closeOnEscapeCallback: () => void = undefined) {
        if (isUpdating) {
            await InternalFunctions.isDoneUpdating();
        }
    ...
    static onInvoke(eventName: string, callback: (...args: any[]) => void) {
        if (ClientEvents[eventName]) {
            console.warn(`[Client] Duplicate Event Name (${eventName}) for triallife.webview.on (WebViewController.onInvoke)`);
