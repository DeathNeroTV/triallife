...
export default class Raycast {
     ...
     * @memberof Raycast
     */
    static performRaycast(start: Vector3, end: Vector3, flags: number, radius: number, useShapeTest = false): [number, boolean, Vector3, Vector3, number] {
        let raycast = null;
        ...
        if (!useShapeTest) {
            raycast = native.startExpensiveSynchronousShapeTestLosProbe(start.x, start.y, start.z, end.x, end.y, end.z, flags, alt.Player.local.scriptID, 7);
        } else {
            raycast = native.startShapeTestCapsule(start.x, start.y, start.z, end.x, end.y, end.z, radius, flags, alt.Player.local.scriptID, 7);
        }
    ...
    static positionFromCamera(flags: number = -1, useShapeTest: boolean = false, radius: number = 5): Vector3 | null {
        ...
        const [didComplete, didHit, position, surfaceNormal, entityHit] = Raycast.performRaycast(start, end, flags, radius, useShapeTest);
    ...
    static isFacingWater(): null | Vector3 {
        ...
        const [hit, position] = native.testProbeAgainstWater(headPosition.x, headPosition.y, headPosition.z, offsetPosition.x, offsetPosition.y, offsetPosition.z);
