...
class InternalFunctions {
    ...
    static async next(node: iCameraNode): Promise<void> {
        ...
        if (cam1) {
            cam2 = native.createCamWithParams('DEFAULT_SCRIPTED_CAMERA', node.pos.x, node.pos.y, node.pos.z, 0, 0, 0, 90, true, 0);

            camNumber = cam2;
            assignCam2to1 = true;
        } else {
            cam1 = native.createCamWithParams('DEFAULT_SCRIPTED_CAMERA', node.pos.x, node.pos.y, node.pos.z, 0, 0, 0, 90, true, 0);
        ...
        if (node.vehicleBone && node.entityToAttachTo) {
            ...
            native.attachCamToVehicleBone(camNumber, node.entityToAttachTo, node.vehicleBone, true, rot.x, rot.y, rot.z, offset.x, offset.y, offset.z, false);
        }

        if (node.pedBone && node.entityToAttachTo) {
            const offset = node.offset ? node.offset : { x: 0, y: 0, z: 0 };
            native.attachCamToPedBone(camNumber, node.entityToAttachTo, node.vehicleBone, offset.x, offset.y, offset.z, true);
        }
...
export class CinematicCam {
    ...
    static demo() {
        alt.setTimeout(() => {
            native.setEntityCoordsNoOffset(alt.Player.local.scriptID, -383.385375976562, -120.65264892578125, 38.68716812133789, false, false, false);
