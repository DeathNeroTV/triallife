...
export default class PedEditCamera {
    ...
    static async create(_scriptID: number, offset: alt.IVector3 = null, _isLocalPlayer = false): Promise<void> {
        ...
        if (!camera) {
            const forwardVector: alt.Vector3 = native.getEntityForwardVector(isLocalPlayer ? alt.Player.local.scriptID : scriptID) as alt.Vector3;
            ...
            camera = native.createCamWithParams('DEFAULT_SCRIPTED_CAMERA', startCamPosition.x, startCamPosition.y, startCamPosition.z, 0, 0, 0, fov, true, 0);
    ...
    static calculateCamOffset(offset: Vector3): Vector3 {
        return native.getOffsetFromEntityInWorldCoords(isLocalPlayer ? alt.Player.local.scriptID : scriptID, offset.x, offset.y, offset.z);
    }
    ...
    static async runQueue() {
        ...
        const camera2 = native.createCamWithParams('DEFAULT_SCRIPTED_CAMERA', startCamPosition.x, startCamPosition.y, startCamPosition.z + zpos, 0, 0, 0, fov, true, 0);
    ...
    static handleControls() {
        ...
        if (Date.now() > timeBetweenAnimChecks) {
            ...
            if (!native.isEntityPlayingAnim(isLocalPlayer ? alt.Player.local.scriptID : scriptID, 'nm@hands', 'hands_up', 3)) {
